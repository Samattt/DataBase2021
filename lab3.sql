...
SELECT code,budget FROM departments ORDER BY budget DESC OFFSET 0 LIMIT 1;
SELECT employees.name, employees.lastname FROM employees, departments
WHERE employees.department = departments.code
AND departments.budget = (SELECT budget FROM departments ORDER BY  budget ASC  OFFSET 0 LIMIT  1);
SELECT name FROM employees WHERE city = 'Almaty'
UNION SELECT name FROM customers WHERE city='Almaty';
SELECT  budget, code FROM departments WHERE budget>60000 ORDER BY  budget ASC, code DESC;
UPDATE  departments SET  budget = budget * 0.9
WHERE budget = ( SELECT DISTINCT budget FROM departments ORDER BY budget ASC OFFSET  0 LIMIT 1);
SELECT name, budget FROM departments WHERE budget<55000;
